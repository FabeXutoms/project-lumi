...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    if (body.password.length < Enum.PASS_LENGTH) {
      throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error!", `password length must be at least ${Enum.PASS_LENGTH} characters`)
    };
    ...
    if (roles.length == 0) {
      throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error!", "roles field contains invalid role ids");
    }
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      return res.sendStatus(Enum.HTTP_CODES.NOT_FOUND).json({ error: "User already exists" });
    }
